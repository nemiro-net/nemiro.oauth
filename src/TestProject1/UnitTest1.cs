...
namespace TestProject1
{
  ...
  public class UnitTest1
  {
    ...
    public void ExecuteRequestTest()
    {
      ...
      var result = OAuthUtility.ExecuteRequest("GET", "http://kbyte.ru/ru/Programming/Articles.aspx", parameters);

      if (result.ToString().IndexOf("OAuth") != -1)
      {
      ...
      parameters = new HttpParameterCollection 
      {
        ...
        { "test", 123 },
        { new HttpUrlParameter("x", "y") },
        { new HttpFormParameter("a", "bc") }
      };
      result = OAuthUtility.ExecuteRequest("POST", "http://foxtools.ru/Http", parameters);

      string source = result.ToString();
      if (source.IndexOf("<table>") != -1)
      {
        Console.WriteLine(source.Substring(source.IndexOf("<table>"), source.IndexOf("</table>") - source.IndexOf("<table>")));
      }
      else
      {
      ...
      result = OAuthUtility.ExecuteRequest("POST", "http://api.foxtools.ru/Screen/json", parameters);
      ...
      using (var fs = new FileStream(Path.Combine(AppDomain.CurrentDomain.SetupInformation.ApplicationBase, "oauth.dat"), FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
      {
        parameters = new HttpParameterCollection 
        {
          { new HttpFile("file1", "test.png", "image/png", fs) },
          { "dd", 20 },
          { "id", 13 },
          { "key", "e91268b7-1162-4044-9236-7191c8c3b5d2" }
        };
